...
Superseded working summary; not an input to the current manuscript or release
bundle. The rates below predate the completed fairness reruns, and the missing
transfer/startup records were subsequently recovered. For current values use
`paper.org`, `tables/`, and `analysis.json`, generated by `report.py` from
`evidence.json`. Historical numbers remain here for traceability.
