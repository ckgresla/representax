"""Isolated ModernVBERT forward probe for Representax or Transformers."""
...
def _representax(
    ...
    if "pixel_values" in inputs:
        ...
        batch = ModernVBERTBatch(
            ...
            pixel_attention_mask=jnp.asarray(inputs["pixel_attention_mask"]),
        )
    else:
...
def _transformers(
    ...
    pixel_values = (
        torch.as_tensor(inputs["pixel_values"], device="cuda", dtype=torch.float32)
        if "pixel_values" in inputs
        else None
    )
    pixel_attention_mask = (
        torch.as_tensor(inputs["pixel_attention_mask"], device="cuda", dtype=torch.long)
    ...
    report.update(
        {
            ...
            "image_count": (
                int(inputs["pixel_values"].shape[1]) if "pixel_values" in inputs else 0
            ),
            "dtype": "float32",
