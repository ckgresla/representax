"""Isolated native or Transformers CLIP forward-performance probe."""
...
def _fingerprint(inputs: dict[str, np.ndarray]) -> str:
    digest = hashlib.sha256(b"clip-text-forward-fp32-v1")
...
def _representax(
    ...
    started = time.perf_counter()
    model = CLIPCheckpointAdapter(normalize_output=True).load(
    ...
    batch = CLIPBatch(
        ...
        text_valid=jnp.ones((inputs["input_ids"].shape[0],), dtype=bool),
    )
...
def _transformers(
    ...
    if transformers.__version__ != "5.6.0":
        raise RuntimeError(
            "CLIP performance requires transformers==5.6.0; "
