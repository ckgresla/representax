"""Pinned numerical parity for the native bidirectional BERT encoder."""
...
def _upstream_python() -> str:
    executable = os.environ.get("REPRESENTAX_BERT_TRANSFORMERS_PYTHON")
    if executable is None:
        pytest.skip("set REPRESENTAX_BERT_TRANSFORMERS_PYTHON for BERT parity")
    return executable
...
def _generate_oracle(directory: Path) -> None:
    subprocess.run(
        [
            _upstream_python(),
            "-m",
            "tests.models.bert.transformers_oracle",
            "--output-directory",
            str(directory),
        ],
        check=True,
...
def oracle_checkpoint(tmp_path_factory):
    directory = tmp_path_factory.mktemp("bert-oracle")
...
def test_pinned_transformers_forward_pooler_and_input_gradient_parity(
    ...
    model = BertCheckpointAdapter(rematerialization="full").load(oracle_checkpoint)
    batch = BertBatch(
        input_ids=jnp.asarray(reference["input_ids"]),
        attention_mask=jnp.asarray(reference["attention_mask"]),
        token_type_ids=jnp.asarray(reference["token_type_ids"]),
    )
    embedded_batch = BertBatch(
    ...
    def parity_outputs(candidate, token_batch, embeddings_batch, vector):
        ...
        def embedded_objective(values):
            values_batch = BertBatch(
                inputs_embeds=values,
                attention_mask=embeddings_batch.attention_mask,
                token_type_ids=embeddings_batch.token_type_ids,
            )
...
def test_pinned_parameter_gradients_and_adamw_update_parity(oracle_checkpoint):
    reference = np.load(oracle_checkpoint / "oracle.npz")
    adapter = BertCheckpointAdapter(rematerialization="full")
    ...
    for name, value in adapter.state_dict(updated).items():
        ...
        if np.linalg.norm(expected_gradient) <= 1e-7:
            # Adam normalizes backend-specific roundoff in analytically null
            # directions such as self-attention key bias. Bound the resulting
            # absolute parameter perturbation without assigning it a direction.
            result = numerical_result(np.asarray(value), expected)
...
def test_native_export_reloads_in_pinned_transformers(oracle_checkpoint, tmp_path):
    ...
    subprocess.run(
        [
            _upstream_python(),
            "-m",
            "tests.models.bert.transformers_reload",
            "--checkpoint",
            str(export),
            "--inputs",
