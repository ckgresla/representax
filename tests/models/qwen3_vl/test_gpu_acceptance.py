"""Physical acceptance probes for Eager Embed V1."""
...
from representax.models.qwen3_vl import Qwen3VLEncoder
...
def _checkpoint() -> Path:
    value = os.environ.get("REPRESENTAX_QWEN3_VL_CHECKPOINT")
    if value is None:
        pytest.skip("set REPRESENTAX_QWEN3_VL_CHECKPOINT for GPU acceptance")
...
def test_real_eager_checkpoint_runs_three_quantized_adapter_updates() -> None:
    if jax.default_backend() != "gpu":
        pytest.skip("real Qwen3-VL training acceptance requires a GPU")
    model, processor = Qwen3VLEncoder.load_from_hf(
        _checkpoint(),
