    ...
    for source_index, row in enumerate(rows):
        ...
        caption = next(
            (
                ...
                if (value := str(item).strip()) and value not in captions
            ),
            None,
        )
        if caption is None:
        ...
        selected.append((source_index, row, caption))
        if len(selected) == count:
            return tuple(selected)
    raise ValueError(f"MSR-VTT contains only {len(selected)} unique usable rows")
...
def _prepare_training(directory: Path, count: int) -> tuple[Path, ...]:
    ...
    records = []
    paths = []
    for source_index, row, caption in rows:
...
def _validate_training_manifest(manifest: Mapping[str, Any]) -> None:
    if not manifest.get("unique_video_ids") or not manifest.get("unique_captions"):
        raise ValueError(
            "video-text training data predates duplicate-free preparation; rebuild it"
        )
...
class VideoTextRetrievalCollator:
    ...
    def __call__(self, rows: Sequence[Mapping[str, Any]]) -> Any:
        ...
        query = self.processor(
            tuple(
                {"video": _representax_video(self.root_directory, str(row["video"]))}
...
def _sentence_transformers_worker(
    *,
    ...
    timer = CudaStepTimer()
    midpoint_callback = (
        _StopAtMidpoint.build(midpoint_step) if platform == "gpu" else None
    )
    trainer = PreflightTrainer(
        ...
        if midpoint_callback is None
        else [midpoint_callback, timer.callback()],
    )
    torch_reset_peak_memory()
    started = time.perf_counter()
    trainer.train()
    ...
    assert midpoint_callback is not None
